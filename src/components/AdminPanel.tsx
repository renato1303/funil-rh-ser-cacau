...
export default function AdminPanel({ onClose }: AdminPanelProps) {
      ...
      if (data) {
        ...
        const mappedLeads: LeadData[] = data.map((row: any) => {
          ...
          return {
            ...
            // Closer qualification fields
            instagram: row.instagram || '',
            tempoAtuacaoCloser: row.tempo_atuacao_closer || row.tempoAtuacaoCloser || row.segmento || '',
            ultimaEmpresa: row.ultima_empresa || row.ultimaEmpresa || row.empresa || '',
            formacaoVendas: row.formacao_vendas || row.formacaoVendas || '',
            quaisFormacoes: row.quais_formacoes || row.quaisFormacoes || '',
            taxaConversao: row.taxa_conversao || row.taxaConversao || '',
            faixaTicketMedio: row.faixa_ticket_medio || row.faixaTicketMedio || row.faturamento || '',
            modeloRemuneracao: row.modelo_remuneracao || row.modeloRemuneracao || '',
    ...
    try {
      await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          timestamp: new Date().toISOString(),
          lead: {
            nome: "Candidato Teste (Painel)",
            email: "candidato@teste.com.br",
            whatsapp: "(11) 98888-7777",
            telefone: "(11) 98888-7777",
            instagram: "@closer.teste",
            tempoAtuacaoCloser: "de 1 a 3 anos",
            tempo_closer: "de 1 a 3 anos",
            ultimaEmpresa: "Empresa Exemplo Growth",
            empresa: "Empresa Exemplo Growth",
            formacaoVendas: "Sim (quais?)",
            quaisFormacoes: "SPIN Selling e Closer de Elite",
            taxaConversao: "28%",
            faixaTicketMedio: "de 5k a 10k",
            modeloRemuneracao: "Fixo mensal + comissão por venda",
            estado: "São Paulo",
            uf: "SP",
            ddd: "11",
            regiao: "Sudeste",
            ...
            utmSource: "instagram",
            utmMedium: "CONJUNTO_TESTE_CLOSERS",
            utmCampaign: "SELECAO_CLOSERS_2026",
            utmContent: "ANUNCIO_VAGA_01",
            utmTerm: "Stories",
            anuncio: "ANUNCIO_VAGA_01",
            conjunto: "CONJUNTO_TESTE_CLOSERS",
            campanha: "SELECAO_CLOSERS_2026",
            adId: "12034567890123456"
          }
        }),
        mode: 'no-cors'
      });
      addLog('Google Sheets', 'success', `Linha de teste para Closer de Vendas enviada com sucesso para a Planilha Google.`);
    } catch (err: any) {
  ...
  const exportCSV = () => {
    ...
    const headers = [
      ...
      'Telefone / WhatsApp',
      'Instagram',
      'Tempo de Atuação Closer',
      'Última Empresa / Projeto',
      'Formação em Vendas',
      'Detalhes Formações',
      'Taxa de Conversão',
    ...
    const rows = leads.map(l => {
      ...
      return [
        ...
        l.tempoAtuacaoCloser || l.segmento || '',
        l.ultimaEmpresa || l.empresa || '',
        l.formacaoVendas || '',
        l.quaisFormacoes || '',
        l.taxaConversao || '',
        l.faixaTicketMedio || l.faturamento || '',
        l.modeloRemuneracao || '',
        scoreFormatted,
        estado,
        uf,
    ...
    link.setAttribute("download", `candidatos_closers_${new Date().toLocaleDateString('pt-BR').replace(/\//g, '-')}.csv`);
                  ...
                  <table className="w-full text-left border-collapse table-auto font-sans antialiased">
                    <thead>
                      <tr className="border-b border-white/10 bg-white/5 text-[11px] font-semibold text-white uppercase tracking-wider">
                        ...
                        <th className="py-4 px-4">Candidato / Contato</th>
                        <th className="py-4 px-4">Tempo Closer</th>
                        <th className="py-4 px-4">Ticket Médio</th>
                          ...
                          <tr key={lead.id} className="hover:bg-white/5 transition-all duration-150">
                            ...
                            <td className="py-4 px-4 space-y-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                <span className="font-bold text-white text-sm tracking-tight">{lead.nome}</span>
                                {lead.instagram && (
                                  <span className="text-[10px] bg-purple-500/10 text-purple-300 border border-purple-500/20 px-1.5 py-0.5 rounded-full font-medium">
                              ...
                              <div className="text-[#A1A1AA] text-[11px]">
                                ...
                                {(lead.ultimaEmpresa || lead.empresa) && (
                                  <span className="text-white/60"> &bull; Ex: {lead.ultimaEmpresa || lead.empresa}</span>
                                )}
                              </div>
                              {phone && (
                            ...
                            {/* TEMPO CLOSER */}
                            <td className="py-4 px-4">
                              <span className="inline-flex px-2 py-0.5 rounded-md text-[11px] font-medium border bg-white/5 border-white/10 text-white">
                                {lead.tempoAtuacaoCloser || lead.segmento || 'Não informado'}
                              </span>
                            </td>
                            
                            {/* TICKET MÉDIO */}
                            <td className="py-4 px-4 text-white font-semibold">
                              <span className="inline-flex px-2 py-0.5 rounded-md text-[11px] font-medium border bg-emerald-500/10 text-emerald-400 border-emerald-500/20">
                                {lead.faixaTicketMedio || lead.faturamento || 'Não informado'}
                              </span>
                            </td>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                          ...
                          <div className="bg-[#121215] border border-white/10 p-3.5 rounded-2xl">
                            <span className="text-[10px] font-semibold text-[#A1A1AA] uppercase tracking-wider block">Instagram</span>
                            <span className="text-sm text-purple-300 font-semibold block mt-1 font-mono">
                              {selectedLead.instagram ? `@${selectedLead.instagram.replace(/^@/, '')}` : 'Não informado'}
                            </span>
                          </div>

                          <div className="bg-[#121215] border border-white/10 p-3.5 rounded-2xl">
                            <span className="text-[10px] font-semibold text-[#A1A1AA] uppercase tracking-wider block">Tempo de Atuação como Closer</span>
                            <span className="text-sm text-brand-cyan font-semibold block mt-1">
                              {selectedLead.tempoAtuacaoCloser || selectedLead.segmento || 'Não informado'}
                            </span>
                          </div>

                          <div className="bg-[#121215] border border-white/10 p-3.5 rounded-2xl">
                            <span className="text-[10px] font-semibold text-[#A1A1AA] uppercase tracking-wider block">Última Empresa ou Projeto</span>
                            <span className="text-sm text-white font-bold block mt-1">
                              {selectedLead.ultimaEmpresa || selectedLead.empresa || 'Não informado'}
                            </span>
                          </div>

                          <div className="bg-[#121215] border border-white/10 p-3.5 rounded-2xl">
                            <span className="text-[10px] font-semibold text-[#A1A1AA] uppercase tracking-wider block">Formação em Vendas</span>
                            <span className="text-sm text-white font-semibold block mt-1">
                              {selectedLead.formacaoVendas || 'Não informado'}
                              {selectedLead.quaisFormacoes ? ` (${selectedLead.quaisFormacoes})` : ''}
                            </span>
                          </div>

                          <div className="bg-[#121215] border border-white/10 p-3.5 rounded-2xl">
                            <span className="text-[10px] font-semibold text-[#A1A1AA] uppercase tracking-wider block">Taxa de Conversão</span>
                            <span className="text-sm text-emerald-400 font-bold block mt-1">
                              {selectedLead.taxaConversao || 'Não informado'}
                            </span>
                          </div>

                          <div className="bg-[#121215] border border-white/10 p-3.5 rounded-2xl">
                            <span className="text-[10px] font-semibold text-[#A1A1AA] uppercase tracking-wider block">Faixa de Ticket Médio</span>
                            <span className="text-sm text-[#00FFC2] font-semibold block mt-1">
                              {selectedLead.faixaTicketMedio || selectedLead.faturamento || 'Não informado'}
                            </span>
                          </div>

                          <div className="bg-[#121215] border border-white/10 p-3.5 rounded-2xl">
                            <span className="text-[10px] font-semibold text-[#A1A1AA] uppercase tracking-wider block">Modelo de Remuneração</span>
                            <span className="text-sm text-amber-400 font-semibold block mt-1">
                              {selectedLead.modeloRemuneracao || 'Não informado'}
                            </span>
                          </div>
                    ...
                    <div>
                      ...
                      <details className="mt-2 text-[11px] text-[#A1A1AA]">
                        <summary className="cursor-pointer hover:text-white transition-colors font-mono font-semibold text-brand-cyan">
                          📋 Ver código atualizado do Google Apps Script (Processo Seletivo Closer de Vendas)
                        </summary>
...
{`function doPost(e) {
  try {
    ...
    var rawScore = (lead && lead.leadScore !== undefined && lead.leadScore !== null) 
      ? lead.leadScore 
      : ((lead && lead.lead_score !== undefined) ? lead.lead_score : 0);
    var scoreFormatted = rawScore + "%";
    ...
    sheet.appendRow([
      new Date(),                                                              // Coluna A: Data/Hora
      lead.nome || lead.Nome || "",                                            // Coluna B: Nome Completo
      lead.email || lead['E-mail'] || "",                                      // Coluna C: E-mail
      lead.whatsapp || lead.telefone || lead.WhatsApp || "",                   // Coluna D: Telefone / WhatsApp
      lead.instagram || lead.Instagram || "",                                  // Coluna E: Instagram (@...)
      lead.tempoAtuacaoCloser || lead.tempo_closer || lead.tempo_atuacao_closer || lead.segmento || "", // Coluna F: Tempo de Atuação Closer
      lead.ultimaEmpresa || lead.ultima_empresa || lead.empresa || "",         // Coluna G: Última Empresa / Projeto
      lead.formacaoVendas || lead.formacao_vendas || "",                       // Coluna H: Formação em Vendas (Sim/Não)
      lead.quaisFormacoes || lead.quais_formacoes || "",                       // Coluna I: Detalhes das Formações
      lead.taxaConversao || lead.taxa_conversao || "",                         // Coluna J: Taxa de Conversão
      lead.faixaTicketMedio || lead.faixa_ticket_medio || lead.ticket_medio || lead.faturamento || "", // Coluna K: Ticket Médio
      lead.modeloRemuneracao || lead.modelo_remuneracao || "",                 // Coluna L: Modelo de Remuneração
      scoreFormatted,                                                          // Coluna M: Score (%)
      lead.estado || lead.Estado || "",                                        // Coluna N: Estado
      lead.uf || lead.UF || "",                                                // Coluna O: UF
      lead.ddd || lead.DDD || "",                                              // Coluna P: DDD
      lead.regiao || lead.Regiao || "",                                        // Coluna Q: Região
      lead.utmSource || lead.utm_source || "",                                 // Coluna R: UTM Source
      lead.utmMedium || lead.utm_medium || "",                                 // Coluna S: UTM Medium (Conjunto)
      lead.utmCampaign || lead.utm_campaign || "",                             // Coluna T: UTM Campaign (Campanha)
      lead.utmContent || lead.utm_content || lead.anuncio || lead.Anuncio || "", // Coluna U: UTM Content (Anúncio)
      lead.utmTerm || lead.utm_term || lead.posicionamento || "",              // Coluna V: UTM Term (Posicionamento)
      lead.adId || lead.ad_id || "",                                           // Coluna W: ID do Anúncio
      lead.id || lead.ID || ""                                                 // Coluna X: ID do Candidato
    ]);
...
function doGet(e) {
  return ContentService.createTextOutput(JSON.stringify({
    "status": "ok", 
    "message": "Webhook Google Apps Script - Processo Seletivo Closer de Vendas ativo!"
  })).setMimeType(ContentService.MimeType.JSON);
}`}
                        </div>
                      </details>
